...
- `spec.json` is always the latest working spec at the root of the repo.
- `versions/spec.x.y.z.json` is a copy of a specific released spec version. Once we put one there, we should treat it as frozen.
- `versions/spec.x.y.latest.json`, `versions/spec.x.latest.json`, and `versions/spec.latest.json` are moving convenience copies for "latest patch in this minor", "latest in this major", and "latest overall".
...
## Quick note about ids in the spec
